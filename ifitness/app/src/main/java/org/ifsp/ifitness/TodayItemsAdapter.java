...
public class TodayItemsAdapter extends RecyclerView.Adapter<TodayItemsAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.amount.setText("Percorrido: "+data.getAmount());
        holder.date.setText("Data: "+data.getDate());
        holder.notes.setText("Obs: "+data.getNotes());


        switch (data.getItem()){
            case "Corrida":
                holder.imageView.setImageResource(R.drawable.ic_baseline_directions_run);
                break;
            case "Natação":
                holder.imageView.setImageResource(R.drawable.ic_baseline_pool);
                break;
            case "Ciclismo":
                holder.imageView.setImageResource(R.drawable.ic_baseline_directions_bike);
                break;
            case "Outro":
                holder.imageView.setImageResource(R.drawable.ic_baseline_sport);
                break;
            default:
    ...
    private void updateData(FirebaseUser user) {
        ...
        updateBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Data data = new Data(item, date, postid, note, amount, user.getEmail().split("@")[0]);
                DatabaseReference reference = FirebaseDatabase.getInstance().getReference().child("percurso").child(FirebaseAuth.getInstance().getCurrentUser().getUid()
                );
